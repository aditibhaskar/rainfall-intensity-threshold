...
# USB MODIFIED TO 15 MINUTE INTERVALS
...
png(filename=paste0("./for_paper_Baisman_iv_", format(Sys.time(), "%d-%b-%Y %H.%M"), ".png"), width=7, height=4, units="in", res=300)
...
legend('topleft', c("Flow event", "Baseflow", "Discharge"), col=c("red", "black", "blue"), pch=c(1,1,NA), lty=c(NA, NA, 1), horiz=TRUE)
title('a. Forested', adj=0)
# plot(RIST$date, RIST$precip.mm, 'h')
dev.off()

## simplified plot
png(filename=paste0("./05-09_Baisman_MI60not.baseflow.cutoff", format(Sys.time(), "%d-%b-%Y %H.%M"), ".png"))#, width=4, height=3, units="in", res=300)
